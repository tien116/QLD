...
public class GiaoVienDAO implements IGiaoVienDAO {
    ...
    public ArrayList<GiaoVien> getAll() {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("select *from tblGiangViens");
                rs = ps.executeQuery();
    ...
    public ArrayList<GiaoVien> findByIDMonHoc(String mamh) {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("select  * from tblGiangViens where fldMaMH = ?");
                ps.setString(1, mamh);
                rs = ps.executeQuery();
                list = new ArrayList<GiaoVien>();
                while (rs.next()) {
    ...
    public GiaoVien addNew(GiaoVien gv) {
        ...
        if (DBConnect.open()) {
            try {
                ...
                ps = DBConnect.cnn.prepareStatement("INSERT INTO tblGiangViens values (?,?,?,?,?,?,?,?)");
    ...
    public GiaoVien updateByID(GiaoVien gv) {
        ...
        if (DBConnect.open()) {
            try {
                ps = DBConnect.cnn.prepareStatement("update tblGiangViens set fldHoTenGV =?,"
                        + "fldMaMH= ?,fldNgaySinh=?,"
    ...
    PreparedStatement ps = null;
        if (DBConnect.open()) {
            ps = DBConnect.cnn.prepareStatement("delete from tblGiangViens where fldMaGV= ?");
    ...
    public ArrayList<GiaoVien> CheckID(String magv) {
      ArrayList<GiaoVien> list = null;
        ...
        if (DBConnect.open()) {
            try {
                psCheck = DBConnect.cnn.prepareStatement("select *from tblGiangViens where fldMaGV=?");
